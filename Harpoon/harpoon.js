...
window.initHarpoonFeature = async () => {
  ...
  const navigateHarpoonList = (direction) => {
    ...
    if (selectedHarpoonIndex === -1 && items.length > 0) {
      ...
      selectedHarpoonIndex =
        (selectedHarpoonIndex - 1 + items.length) % items.length;
    }
  ...
  const loadHarpoonedTabs = async () => {
    ...
    if (urlToFocus) {
      ...
      if (selectedHarpoonIndex === -1) {
        // Only set to 0 if no item is already selected
  ...
  const renderHarpoonedTabs = () => {
    ...
    harpoonedTabs.forEach((harpoonedTab, index) => {
      ...
      favicon.alt = "icon";
      favicon.src =
        harpoonedTab.favIconUrl || chrome.runtime.getURL("img/icon.png"); // Fallback icon
