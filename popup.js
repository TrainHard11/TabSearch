document.addEventListener("DOMContentLoaded", () => {
    ...
    const focusOrCreateTabByUrl = (url, exactMatch = false) => {
        // Construct query options based on exactMatch parameter
        const queryOptions = exactMatch ? { url: url } : { url: `${url}*` };

        chrome.tabs.query(queryOptions, (tabs) => {
            if (tabs.length > 0) {
                // If tabs are found, switch to the first one
    ...
    const getSettingsDOMElements = () => {
        ...
        searchMarksCheckbox =
            settingsContentContainer.querySelector("#searchMarksCheckbox");
        customTabInputs = [];
        customTabExactMatchCheckboxes = [];
        for (let i = 1; i <= 7; i++) { // Corrected loop condition to include all 7 custom tabs
    ...
    const renderResults = (resultsToRender, suggestedIndex = 0) => {
        ...
        resultsToRender.forEach((item, index) => {
            ...
            if (item.type === "tab") {
                ...
                if (item.exactMatch) {
                    titleSpan.innerHTML += ' <span class="exact-match-label">[Exact Match]</span>';
                }
                urlSpan.innerHTML = highlightText(item.url || "", currentQuery);
        ...
        if (e.key === "ArrowDown" || (e.altKey && e.key === "j")) {
            ...
            if (selectedIndex !== -1 && filteredResults[selectedIndex]) {
                ...
                if (selectedItem.type === "tab") {
                    ...
                    // NEW: Pass the exactMatch property to focusOrCreateTabByUrl
